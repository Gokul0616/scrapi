...
# ============= Scraper Creation & Marketplace Routes =============
...
async def get_template_by_type(template_type: str, current_user: dict = Depends(get_current_user)):
...
@router.post("/actors/validate-schema")
async def validate_input_schema(schema: dict, current_user: dict = Depends(get_current_user)):
    ...
    if "properties" not in schema:
        return {"valid": False, "errors": ["Schema must have 'properties' field"]}
    
    # Validate properties
    errors = []
    for key, prop in schema.get("properties", {}).items():
        if "type" not in prop:
            errors.append(f"Property '{key}' must have a 'type' field")
        if "title" not in prop:
            errors.append(f"Property '{key}' should have a 'title' field")
...
@router.post("/actors/{actor_id}/fork", response_model=Actor)
async def fork_actor(actor_id: str, current_user: dict = Depends(get_current_user)):
    """Clone/fork an existing actor."""
    # Get original actor
    original = await db.actors.find_one({"id": actor_id})
    if not original:
        raise HTTPException(status_code=404, detail="Actor not found")
    
    # Check if user can access this actor (must be public or owned by user)
    if not original.get('is_public') and original.get('user_id') != current_user['id']:
        raise HTTPException(status_code=403, detail="Cannot fork private actor")
    ...
    forked_actor = Actor(
        ...
        icon=original.get('icon', '🕷️'),
        category=original.get('category', 'General'),
        type=original.get('type', 'prebuilt'),
        code=original.get('code'),
        input_schema=original.get('input_schema', {}),
        tags=original.get('tags', []),
        readme=original.get('readme'),
        template_type=original.get('template_type'),
        visibility='private',
        status='draft',
        fork_from=actor_id,
        author_name=current_user.get('username'),
...
@router.get("/marketplace", response_model=List[Actor])
async def get_marketplace_actors(
    ...
    """Get public marketplace actors with filters."""
    query = {
        "status": "published",
        "visibility": "public"
    }
    ...
    actors = await db.actors.find(query, {"_id": 0}).sort("runs_count", -1).to_list(1000)
...
@router.patch("/actors/{actor_id}/publish", response_model=Actor)
async def publish_actor(
    actor_id: str,
    publish_data: ActorPublish,
    current_user: dict = Depends(get_current_user)
):
    """Publish a draft actor."""
    actor = await db.actors.find_one({"id": actor_id, "user_id": current_user['id']})
    if not actor:
        raise HTTPException(status_code=404, detail="Actor not found")
    
    # Update actor with publish data
    update_data = {
        "status": "published",
    ...
    if publish_data.readme:
        update_data["readme"] = publish_data.readme
    if publish_data.tags:
        update_data["tags"] = publish_data.tags
    
    # Set is_public based on visibility
    update_data["is_public"] = publish_data.visibility == "public"
